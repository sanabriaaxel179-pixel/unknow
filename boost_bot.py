...
@bot.tree.command(name="bulk_gen_license", description="Generate multiple Bootbot licenses at once (Staff Only)", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(duration="The duration of the license", amount="How many keys to generate (Max 20)")
@app_commands.choices(duration=[
    app_commands.Choice(name="1 Day", value="1day"),
    app_commands.Choice(name="1 Week", value="1week"),
    app_commands.Choice(name="1 Month", value="1month"),
...
async def bulk_gen_license(interaction: discord.Interaction, duration: str, amount: int):
    has_permission = False
    if interaction.user.guild_permissions.administrator:
        has_permission = True
    elif hasattr(interaction.user, "roles"):
    ...
    if amount < 1 or amount > 20:
        await interaction.response.send_message("❌ You can only generate between 1 and 20 keys at a time.", ephemeral=True)
        return

    # Defer response since it might take a second to generate and save multiple keys
    await interaction.response.defer()
    ...
    embed = discord.Embed(title=f"🔑 {amount}x Keys Generated", description=f"Duration: **{duration.capitalize()}**\n\n```\n{keys_string}\n```", color=discord.Color.blue())
    embed.set_footer(text=f"Generated by {interaction.user} | Ready to paste into SellApp!")
